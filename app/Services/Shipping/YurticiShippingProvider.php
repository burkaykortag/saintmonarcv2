...
class YurticiShippingProvider implements ShippingProviderInterface
{
    ...
    {
        $this->username = $username ?? (string)getenv('YURTICI_USER');
        $this->password = $password ?? (string)getenv('YURTICI_PASS');
    }
    ...
    public function createShipment(array $shipmentData): array
    {
        if (!$this->isConfigured()) {
            return [
                ...
                'message' => 'Yurtiçi Kargo credentials missing in .env (YURTICI_USER / YURTICI_PASS)',
                'provider' => 'Yurtici',
                'requires_credentials' => true
            ];
        }

        $trackingNo = 'YK-' . time() . rand(100, 999);
        return [
            'success' => true,
            'status' => 'created',
