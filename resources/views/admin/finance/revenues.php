...
<div class="mb-4">
    <?= ComponentHelper::breadcrumb(['Yönetim Paneli' => url('/admin'), 'Finans & Muhasebe' => url('/admin/finance'), 'Gelir Yönetimi' => '#']) ?>
    <div class="d-flex justify-content-between align-items-center mt-2 flex-wrap gap-2">
        <h2 class="text-white font-weight-700 m-0" style="font-size: 26px;">Gelir Yönetim Sistemi</h2>
...
<div class="row g-4 text-white">
    ...
    <div class="col-lg-8">
        <div class="card p-4 border-0" style="background: rgba(255,255,255,0.02); border: 1px solid var(--sm-border) !important; border-radius: 20px;">
            <h4 class="text-white font-weight-600 mb-3 fs-6"><i class="bi bi-list-check text-warning me-2"></i>Gelir Kayıtları (Tahsilatlar)</h4>
                ...
                <table class="table align-middle text-white table-borderless fs-7">
                    ...
                    <tbody>
                        <?php if (empty($revenues)): ?>
                            <tr>
                                <td colspan="5" class="text-center text-muted py-4">Kayıtlı gelir bulunmuyor.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($revenues as $rev): ?>
                                <tr>
                                    <td><span class="badge bg-secondary"><?= htmlspecialchars($rev['category_name']) ?></span></td>
                                    <td><?= htmlspecialchars($rev['description'] ?? '-') ?></td>
                                    <td><?= date('d.m.Y', strtotime($rev['revenue_date'])) ?></td>
                                    <td>₺<?= number_format((float)$rev['tax_amount'], 2) ?></td>
                                    <td class="text-end font-weight-600 text-success">₺<?= number_format((float)$rev['amount'], 2) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
    ...
    <div class="col-lg-4">
        <div class="card p-4 border-0" style="background: rgba(255,255,255,0.02); border: 1px solid var(--sm-border) !important; border-radius: 20px;">
            <h4 class="text-white font-weight-600 mb-3 fs-6">Yeni Dış Gelir Girişi</h4>
            <form action="<?= url('/admin/revenues/create') ?>" method="POST">
                <input type="hidden" name="csrf_token" value="<?= $csrfToken ?>">

                <div class="mb-3">
                    <label class="form-label text-muted fs-7 mb-1">Gelir Kategorisi</label>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="revenue_date" required class="search-input w-100 text-white" value="<?= date('Y-m-d') ?>">
                </div>

                <div class="mb-3">
                    <label class="form-label text-muted fs-7 mb-1">Açıklama</label>
                    <textarea name="description" class="search-input w-100 text-white" rows="2" placeholder="Gelir açıklaması..."></textarea>
                </div>

                <button type="submit" class="btn btn-warning text-dark border-0 fs-7 w-100 py-3 font-weight-700 mt-2">Geliri Kaydet</button>
            </form>
        </div>
    </div>
</div>
